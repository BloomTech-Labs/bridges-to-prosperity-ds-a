...
cols = ['sector_id','cell_id','village_id','community_served_1_id','community_served_2_id','community_served_3_id','community_served_4_id','community_served_5_id'] #, 'col_2', 'col_3', 'col_4']
for col in cols:
   df[col] = df[col].apply(lambda x: int(x) if x == x else 9999999999)

df['project_code'].iloc[534:535]=None # Bad character; alphanumeric value in integer field.
